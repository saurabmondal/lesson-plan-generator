...
with st.sidebar:
    ...
    # Auto-load from Streamlit secrets if available (deployed app)
    default_key = ""
    try:
...
if generate_clicked:
    ...
    with st.spinner("⚡ Groq is generating 3 Bloom's Taxonomy lesson plans..."):
        try:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "No markdown code fences, no explanations, no text outside the JSON object."
                        ),
                    },
                    {"role": "user", "content": _build_prompt(
                        subject, class_level, chapter, topic, duration, context
                    )},
                ],
                temperature=0.7,
                max_tokens=8000,
            ...
            lesson_plans = _parse_lesson_plans(raw)
...
# ══════════════════════════════════════════════════════════════════════════════
#  HELPER FUNCTIONS  (defined after all Streamlit calls to avoid re-run issues)
# ══════════════════════════════════════════════════════════════════════════════

def _build_prompt(subject, class_level, chapter, topic, duration, context):
  ...
  "plans": [
    {{
      ...
      "evaluation": [
        ...
        {{"question": "Question at Evaluate/Create level?", "bloom_level": "Evaluate", "type": "Long Answer"}}
      ],
...
def _parse_lesson_plans(raw_text: str) -> list:
    text = raw_text.strip()
...
def _display_plan(plan: dict, num: int):
    if "raw" in plan:
    ...
    with st.expander("📌 Instructional Objectives — Bloom's Taxonomy", expanded=True):
        for io in plan.get("instructional_objectives", []):
            ...
            st.markdown(
                f'<div style="background:{bg};border-radius:8px;padding:0.5rem 0.9rem;margin:0.3rem 0;">'
                f'{_bloom_tag(level)} &nbsp; {io.get("objective","")}'
                f'</div>',
                unsafe_allow_html=True,
            )
